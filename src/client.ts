import { client, type NetworkMessage } from "snack:client";
...
style.textContent = `
  ...
  * { box-sizing: border-box; }
  ...
  #game, #game-display { display: block; position: absolute; inset: 0; width: 100%; height: 100%; touch-action: none; cursor: crosshair; }
  #hud { position: fixed; inset: 0; pointer-events: none; text-transform: uppercase; letter-spacing: .12em; }
...
let sceneTexture: Texture | undefined;
let sceneSprite: Sprite | undefined;
let phosphorFilter: Filter | undefined;
...
async function readDatagrams(): Promise<void> {
  try {
    while (true) {
      ...
      const message = event.json<unknown>();
...
async function readSnapshots(): Promise<void> {
  try {
    while (true) {
...
function sendInput(): void {
  ...
  void client.datagrams.send(message as unknown as NetworkMessage);
}
...
function sendAction(action: ActionMessage["action"], value?: string): void {
  ...
  const message = value ? { type: "action", action, value } : { type: "action", action };
  void client.datagrams.send(message as unknown as NetworkMessage);
}
...
function render(now: number): void {
  const dt = Math.min(0.035, Math.max(0.001, (now - lastFrameAt) / 1000));
  lastFrameAt = now;
  updatePrediction(dt);
  updateEffects(dt);
  ...
  sceneTexture?.source.update();
  if (phosphorFilter) {
    phosphorFilter.resources.timeUniforms.uniforms.uTime = now / 1000;
  }
  requestAnimationFrame(render);
}
...
async function initializePixi(): Promise<void> {
  ...
  const fragment = `
    ...
    void main(void) {
      ...
      float split = (0.45 + edge * 1.8) * uInputSize.z;
      vec3 color;
      color.r = texture(uTexture, uv + vec2(split, 0.0)).r;
      color.g = texture(uTexture, uv).g;
      color.b = texture(uTexture, uv - vec2(split, 0.0)).b;
      float scan = 0.965 + 0.035 * sin(uv.y * uInputSize.y * 3.14159 + uTime * 4.0);
      float vignette = smoothstep(0.82, 0.24, edge);
      color *= scan * mix(0.76, 1.0, vignette);
      color += max(color - 0.72, 0.0) * 0.22;
  ...
  const display = app.canvas as HTMLCanvasElement;
  display.id = "game-display";
  root.insertBefore(display, required<HTMLElement>("#hud"));
  canvas.style.display = "none";
  bindInputSurface(display);
  inputSurface = display;
  sceneTexture = texture;
  sceneSprite = sprite;
  phosphorFilter = filter;
  pixiApp = app;
}
...
function getView(): { scale: number; offsetX: number; offsetY: number } {
  ...
  const halfWidth = Math.min(WORLD_WIDTH / 2, width / scale / 2);
  const halfHeight = Math.min(WORLD_HEIGHT / 2, height / scale / 2);
  const centerX = clamp(cameraX, halfWidth, WORLD_WIDTH - halfWidth);
  const centerY = clamp(cameraY, halfHeight, WORLD_HEIGHT - halfHeight);
  return {
    scale,
...
function isSnapshot(value: unknown): value is SnapshotMessage {
  ...
  return (
    ...
    Array.isArray(value.motherships) &&
    Array.isArray(value.asteroids) &&
    Array.isArray(value.salvage) &&
